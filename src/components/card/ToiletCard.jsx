...
function ToiletCard({ setIsLoginOpen, setIsModalOpen, setModalData, toilet }) {
	...
	const isFavorite = favoriteToilets?.some(
		(fav) => fav._id.toString() === toilet._id.toString(),
	);
	...
	return (
		<div key={toilet._id} className={`${styles.toilet} box`}>
			...
			<div className={`${styles.favoriteBtn}`}>
				<button
					className={`btnLg ${styles.favoriteBtn} ${isFavorite && "isFavorite"}`}
					onClick={() =>
						toggleFavorite({
							setIsLoginOpen,
							setIsModalOpen,
